...
| CERTAIN-FB-001 | Challenge comparison must tolerate Dictation's numeric rendering. | `FEEDBACK_LEDGER.md`; `__tests__/challenge.test.ts` | Dictation returned `code 16` for spoken `six one`; literal word comparison would fail. Canonical challenge matching now maps digit words and splits compound numeric tokens. | Live response reproduced the shape; regression test passes. | `BUG` resolved | Certain implementation fix, not AssemblyAI feedback. |
